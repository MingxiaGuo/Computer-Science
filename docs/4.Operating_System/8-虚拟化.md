...
通过内核的KVM技术和EPT技术，加速虚拟机对于物理CPU和内存的使用，我们称为硬件辅助虚拟化。
...
<domain type='kvm'>   # 如果是Xen，则type=‘xen’
  ...
  <memory unit='KiB'>524288</memory> #虚拟机内存大小
  <currentMemory unit='KiB'>524288</currentMemory>  // 虚拟机在开机时分配的内存大小，,memory这两个值最好设成一样
  ...
  <os>
   ...
   <boot dev='hd'/>  //启动介质，第一次需要装系统可以选择cdrom光盘启动;#启动方式,cdrom为从光盘启动(ISO镜像)，后面会再次修改启动方式
  ...
  <devices>   //设备配置
   ...
   <disk type='file' device='disk'> //硬盘
      ...
      <source file='/path/mydisk-img.qcow2'/> #虚拟硬盘
   ...
   <disk type='file' device='cdrom'>//光盘
      ...
      <source file='/path/CentOS-7-x86_64-DVD-2009.iso'/> #ISO镜像路径
   ...
   <graphics type='vnc' port='5900' autoport='yes' listen='0.0.0.0' keymap='en-us'>  //配置vnc，windows下可以使用vncviewer登录，5900为端口号,获取vnc端口号：virsh vncdisplay vm0
...
<domain type='kvm'>　　#使用kvm创建
  ...
  <vcpu placement='static'>2</vcpu>      #cpu个数，查看物理机可用CPU个数：cat /proc/cpuinfo |grep processor | wc -l 
  <os>
    <type arch='x86_64' machine='pc-i440fx-1.5'>hvm</type> //arch指出系统架构类型，machine 则是机器类型，查看机器类型：qemu-system-x86_64 -M ?
...
* **raw**: KVM(virt-install安装时)和XEN默认的格式。因为其原始，有很多原生的特性，例如直接挂载也是一件简单的事情。
* **cow**: copy-on-write格式，昙花一现
* **qcow**: QEMU早期的copy-on-write格式，过渡性方案
* **qcow2**: 对于qemu-kvm来讲，qcow2是一种常见的文件格式。它有精妙的格式设计，从而适应虚拟化的场景. openstack默认的格式
...
该格式是目前很多开源虚拟化软件的首选格式，原因很简单，因为其支持snapshot快照。该格式具有如下特点：

- 更小的存储空间，即使是不支持_holes_的文件系统也可以（这下du -h和ls -lh看到的就一样了）
