文件系统：持久存储数据的子系统，负责把用户数据存到磁盘硬件中，断电了数据也不会丢失。可以持久化的保存文件。
...
* 文件基本操作：create, delete, open, close, read, write 系统调用
* 磁盘空间管理
  * 对非空闲磁盘块管理(存放了文件数据的磁盘块)—— 文件的物理结构/文件分配方式
  * 对空闲磁盘块管理 —— 文件存储空间管理：操作系统如何管理外存中的空闲块
* 文件共享
* 文件保护

# 文件系统操作

df：列出文件系统已挂载磁盘的总容量，使用量；针对整个文件系统
du：文件系统的目录使用量
lsblk: 列出所有可用块设备的信
fdisk: 较旧的linux 磁盘分区工具，用于传统的 MBR((Master Boot Record))分区表
gdisk：更现代的linux 下磁盘分区工具，用于现代的 GPT(GUID Partition Table) 分区表,GPT 分区表支持更大的磁盘容量和更多的分区数量。
mkfs
mount：
umont：
parted -l
...
```sh
[root@iZf8ziznc480ufao5wesaaZ gmx]# df -h
Filesystem      Size  Used Avail Use% Mounted on
devtmpfs        925M     0  925M   0% /dev
tmpfs           945M     0  945M   0% /dev/shm
tmpfs           945M  768K  944M   1% /run
tmpfs           945M     0  945M   0% /sys/fs/cgroup
/dev/vda3        40G   11G   27G  30% /
tmpfs           945M     0  945M   0% /tmp
/dev/vda2       200M  5.8M  194M   3% /boot/efi
overlay          40G   11G   27G  30% /var/lib/docker/overlay2/8a54dae3e7f418cf491df64e3b75e7acf8e796521f6b1942a6a4653cd039686b/merged
shm              64M  1.1M   63M   2% /var/lib/docker/containers/e8df2a59f92d9217d2a09a58c0666f29f130db7c8dad5ba1057baaa9b3a30e93/mounts/shm
overlay          40G   11G   27G  30% /var/lib/docker/overlay2/a51901edf48bf1dff21f34fbc3b38393ebbb41d49b9244b329d706be7882d8b5/merged
shm              64M     0   64M   0% /var/lib/docker/containers/4d9ff8995e97d38f7b4dc3dea28fe63fd21cdb3becd144fbbc4b08c4373d33a2/mounts/shm
overlay          40G   11G   27G  30% /var/lib/docker/overlay2/ddcb8cb76994c4f83c9c6afcdd7a455dd5cb79f72718620054fbc219c97e474d/merged
shm              64M     0   64M   0% /var/lib/docker/containers/e8dfe404f75a0415a9c5049cb36380e2c6a09168ab2241a1d3ada863c68da3b5/mounts/shm
tmpfs           189M     0  189M   0% /run/user/1000
```

> Filesystem: 表示扇区，也就是你划分磁盘时所分的区
> Mounted on： 表示该分区（扇区）所挂载的目录，即挂载点


```sh
[root@iZf8ziznc480ufao5wesaaZ gmx]# du -sh /var/log
212M	/var/log
...
[root@iZf8ziznc480ufao5wesaaZ gmx]# fdisk -l
...
**文件定义**: 以计算机硬盘(磁盘)为载体的存储在计算机上的信息集合
文件表示的范围/包含的内容: 源程序、二进制代码、文本文档、数据、表格、声音和图像等。
**文件属性**：描述文件状态的一组信息

* 文件名：为了方便用户找到文件，同一目录下不能重名
* 标识符：一个系统内的各文件标识符唯一，对用户来说毫无可读性，因此表示服只是操作系统用于区分各个文件的一种内部名称
* 文件类型：.txt, .csr, .pdf,
* 物理位置：文件存放路径
* 大小
* 保护信息:用户对该文件的存取权限
...
* 复制文件：先创建空文件再把源文件读入内存，将内存中数据写入新文件；可以由上面简单系统调用组合实现复杂的操作
* 文件重定位（寻址）
* 截断文件
  ![](image/文件属性.png)
...
* 文件的逻辑结构：文件内部数据如何组织，人看到的文件长什么样？
  * 无结构文件（流式文件）: 文本，图片，代码;有而无结构文件则被看成一个字符流。由一些二进制或字符流组成。
    * 以字节(byte)为单位
    * 没有具体结构，因此也不用探讨无结构文件的“逻辑结构”问题
    * 采用穷举方式搜索
  * 有结构文件（记录式文件）：excel表格，数据库表文件;结构文件是由若干个相关记录组成，记录是一组相关数据项组成。数据项是文件系统中最基本的数据单位。根据记录长度分为定长记录和可变长记录。根据有结构文件中的各条记录在逻辑上如何组织，可分为三类：
    * 顺序文件：链表(变长)，数组(定长)；文件中记录一个个按顺序排列(逻辑上), 各记录在物理上可顺序存储或链式存储
      * 根据记录顺序是否与关键字有关，分为：
        * 串结构：记录之间的顺序与关键字无关，按记录存入时间决定记录的顺序
      ...
      * 根据物理存储方式分
        ...
        * 顺序存储：
          ...
          * 定长记录：
            ...
            * 若采用顺序结构，可快速找到某关键字对应的记录（如折半查找）
    * 索引文件：索引表，可以快速定位，又可变长
    * 索引顺序文件：一个索引表项对应一组记录
    * 直接文件或散列文件（Hash File）：MongoDB数据库系统，键值映射关系
* 文件的物理结构：文件如何存放在外存中，计算机看到的文件长什么样？
* 文件的目录结构：文件之间如何组织，通过目录管理文件，FCB的有序集合为文件目录，一个FCB是一个文件目录项
  * **文件控制块**(FCB)：存储文件和目录信息的数据结构
    * 基本信息：文件名，物理地址，逻辑结构，物理结构
    * 存取控制信息：是否刻度
    * 使用信息：创建/修改信息
  * 索引节点：对文件名建立索引表，对文件控制块的优化(对FCB的改进)，
    * 除了文件名之外的文件描述信息都放到索引结点中，每个文件对应一个索引节点
    * 目录项中包含文件名，索引节点指针，因此每个目录项长度大幅减小
    * 由于目录项长度减小，因此每个磁盘块可存放更多目录项，因此检索文件时磁盘I/O次数减少了很多
  * 目录结构：目录和目录直接的关系，多级目录，每个目录有自己的FCB和索引节点
    * 单级目录结构: 不允许文件重名
    * 两级目录结构：主文件目录(Master File Directory) + 用户文件目录(User File Directory)
    * 多级目录结构（树形目录结构）:现代操作系统常用的目录结构，树形结构很方便地对文件进行分类，层级结构清晰，但不便于实现文件的共享，为此，提出“无环图目录结构”
    * 无环图目录结构: 在树形结构基础上增加一些指向同一节点的有向边，使整个目录成为一个有向无环图，方便在多个用户间共享文件。为共享节点设置一个共享计数器，计数器为0时才真正删除该节点。

> 类似于数据结构的 “逻辑结构”和“物理结构”:
> 如“线性表〞就是一种逻辑结构，在用户角度看来，线性表就是一组有先后关系的元素序列，如 a,b,c,d,e,....
> ‘线性表〞这种逻辑结构可以用不同的物理结构实现，如：顺序表/链表。顺序表的各个元素在逻辑上相邻，在物理上也相邻：而链表的各个元素在物理上可以是不相邻的。因此，顺序表可以实现"随机访问链表〞无法实现随机访问。而“链表”无法实现随机访问。
...
* 文件共享：使多个用户可以共享使用一个文件
  * 硬链接（索引节点）：基于索引节点的共享方式
  * 软链接（符号链）：利用符号链实现文件共享
* 文件保护：文件保护是为了保护文件不受破坏和非法访问，加密保护的成本较高，加密保护比口令保护更安全，可以直接对文件数据进行加密，保证不同用户对文件有不同的操作权限
  * 口令保护：针对文件本身，在访问文件之前需要输入口令，在索引节点加密码；
  * 加密保护：针对文件本身，直接对文件数据进行加密；针对文件本身
  * 访问控制：针对不同用户的操作进行限制，对用户和操作的内容，限制不同用户的读写操作
...
* 文件是存储在磁盘等存储介质上的数据的集合，通常包含一个文件名和文件扩展名。
* 文件可以是文本文件、二进制文件、图像文件、音频文件等。

1. **目录（Directory）** ：

* 目录是一个文件系统的结构，用于组织和管理文件。目录可以包含文件和子目录。
* 目录结构可以是单层结构（所有文件在一个目录中）或多层结构（文件在不同的子目录中）。

1. **路径（Path）** ：

* 路径是用于标识文件或目录位置的字符串。路径分为绝对路径和相对路径。
...
* 创建、删除、读写和修改文件。
* 设置和获取文件属性，如文件名、大小、类型、创建时间、修改时间等。

1. **目录管理** ：

* 创建和删除目录。
* 组织目录结构，支持层级目录（文件夹）管理。
* 导航目录树，支持路径解析。

1. **文件访问控制** ：

* 管理文件和目录的访问权限，防止未经授权的访问。
* 典型的权限控制包括读、写和执行权限。
...
不同操作系统和存储设备支持不同的文件系统类型。以下是一些常见的文件系统：

1. **FAT（File Allocation Table）** ：

* 一种较早的文件系统，广泛用于小型存储设备（如U盘、存储卡）。
* FAT32是其常见版本，支持最大4GB的单个文件和2TB的分区。

1. **NTFS（New Technology File System）** ：

* 由微软开发，用于Windows操作系统。
* 支持大文件和大容量分区，具有强大的安全性和文件恢复能力。

1. **EXT（Extended File System）** ：

* Linux操作系统的标准文件系统。常见的版本有EXT2、EXT3、EXT4。
* EXT4是目前最常用的版本，提供了日志功能和高性能。

1. **HFS+和APFS** ：

* 由苹果公司开发，用于macOS操作系统。
* APFS（Apple File System）是最新版本，优化了SSD存储设备的性能和安全性。

1. **exFAT（Extended File Allocation Table）** ：

* 主要用于闪存存储设备，兼容性好，支持大于4GB的单个文件。
...
* 设备管理程序模块
  ![](image/文件系统层次结构.png)
  点击“创建文件”按钮，图形交互进程在背后调用“create系统调用”，
  双击.txt文件，
...
目录实现：目录也是文件，但实现方式与文件不同，目录的结构本质是文件和文件夹的文件控制块的集合，目录的实现即这个集合的数据结构如何实现。目录本身是一个有结构的文件(由记录组成)。目录只存储文件名和索引节点指针。

* 线性列表：线性表实现方式包括链表和数组，每个节点都是一个文件控制块。  数组和链表都不适合存储文件索引，希表是更好的选择。
  * 链表：增删改查简单，查找效率低，需遍历所有节点，
  * 数组：查找简单，但无法定位所访问的文件是第几个，仍需要遍历所有数据
* 哈希表：哈希表通过索引和文件名进行映射,可以快速定位索引,提高效率。但是,哈希表可能会出现重名的情况,需要解决哈希冲突，需要优化算法并设计好存储结构。使用再哈希解决冲突。哈希表本质也是数组，哈希表可以快速定位索引。

![](image/文件目录实现.png)

## 4.3.2 文件实现

### 4.3.2.1 文件的物理结构/文件分配方式
...
虚拟文件系统VFS：
* 屏蔽下层各种不同类型文件系统的实现细节和差异，如fas32格式，ext3格式。
* 将各种文件系统的公共部分抽出来。形成一个抽象层。
* 对用户而言，VFS提供文件系统的系统调用接口
* 对具体文件系统而言，VFS使用统一外部接口屏蔽实现细节，对文件操作不需关心下层文件系统的类型，不关心具体存储介质。
